...
var popup_disableBodyScroll = popup_bodyScrollLock.disableBodyScroll;
  ...
  init: function init() {
    ...
    openPopupButtons.forEach(function (button) {
      button.addEventListener('click', function (e) {
        ...
        if (type !== 'policy') {
          popup_disableBodyScroll(targetElement);
        }
    ...
    */
    ...
    $('select').each(function () {
      ...
      $styledSelect.click(function (e) {
        ...
        var height = $(this).next('ul.select-options')[0].clientHeight;
        var selectF = $('.filter__select')[0];
        selectF.style.marginBottom = "".concat(height, "px");
      });
      $listItems.click(function (e) {
        ...
        $list.hide();
        var selectF = $('.filter__select')[0];
        selectF.style.marginBottom = "0px";
...
;// CONCATENATED MODULE: ./src/modules/filter.js
// import 'sticksy';

/* harmony default export */ var filter = ({
  init: function init() {
    /*     const stickyEl1 = document.querySelector('.js-sticky-widget');
        if(!stickyEl1) return;
    ...
    var filterIcon = document.querySelector('.filter-icon-mob');
    var hidden = document.querySelector('.filter-hidden');
    if (!filterIcon || !hidden) return;
    ...
    var resetBtn = document.querySelector('.filter__reset');
    var select = document.querySelector('select');
    var progress = document.querySelectorAll('.progress');
    resetBtn.addEventListener('click', function () {
      var selected = document.querySelector('.is-selected');
      var selectedTxt = document.querySelector('.select-styled');
      selectedTxt.textContent = select.options[0].textContent;
...
;// CONCATENATED MODULE: ./src/modules/range.js
/* harmony default export */ var range = ({
  init: function init() {
    ...
    var errorText = document.querySelector(".price-input .error-text");
    var priceGap = 1;
    priceInput.forEach(function (input) {
      // input.value = String( input.value).replace(/(\d)(?=(\d{3})+([^\d]|$))/g, '$1 ');
      input.addEventListener("input", function (e) {
        var minPrice = parseInt(priceInput[0].value),
          maxPrice = parseInt(priceInput[1].value);
        var isInRangeMin = minPrice <= rangeInput[1].max && minPrice >= rangeInput[1].min;
        var isInRangeMax = maxPrice <= rangeInput[1].max && maxPrice >= rangeInput[1].min;

        // console.log('minPrice', minPrice, 'maxPrice', maxPrice);

        if (isInRangeMin === false || isInRangeMax === false) {
          errorText.classList.add('active');
          errorText.textContent = "\u0432\u0432\u0435\u0434\u0438\u0442\u0435 \u0447\u0438\u0441\u043B\u043E \u043E\u0442 ".concat(rangeInput[1].min, " \u0434\u043E ").concat(rangeInput[1].max);
        } else {
        ...
        if (maxPrice - minPrice >= priceGap && maxPrice <= rangeInput[1].max) {
          ...
          if (e.target.className === "input-min") {
            ...
            var test = minPrice / rangeInput[0].max * 100 + "%";
            var test2 = minPrice / rangeInput[0].max * 100 - 8 + "%";
            if (isInRangeMin === false) {
              range.style.left = 0;
            } else if (minPrice < 95) {
              range.style.left = test2;
            } else {
              range.style.left = test;
            }
          } else {
            rangeInput[1].value = maxPrice;
            var _test = 100 - maxPrice / rangeInput[1].max * 100 + "%";
            var _test2 = 100 - maxPrice / rangeInput[1].max * 100 + 6 + "%";
            if (maxPrice < 100) {
              range.style.right = _test2;
            } else {
              range.style.right = _test;
            }
          }
        }
      });
      input.addEventListener('blur', function (e) {
        console.log('dfdf', input);
        var progress = document.querySelectorAll('.progress');
        if (input.className === "input-min" && errorText.classList.contains('active')) {
          input.value = rangeInput[0].min;
          errorText.classList.remove('active');
        } else if (input.className === "input-max" && errorText.classList.contains('active')) {
    ...
    rangeInput.forEach(function (input) {
      input.addEventListener("input", function (e) {
        ...
        if (maxVal - minVal < priceGap) {
          ...
          var test = range.style.left = minVal / rangeInput[0].max * 100 + "%";
          var test2 = range.style.left = minVal / rangeInput[0].max * 100 - 8 + "%";
          if (minVal > 95) {
            range.style.left = test;
          } else {
            range.style.left = test2;
          }
          var tesRight = 100 - maxVal / rangeInput[1].max * 100 + "%";
          var testRight2 = 100 - maxVal / rangeInput[1].max * 100 + 8 + "%";
          if (maxVal < 80) {
            range.style.right = testRight2;
          } else {
            range.style.right = tesRight;
          }
        }
      });
    });
    ...
    // second
    var priceContainer = document.querySelector('.filter__range._sq');
    if (!priceContainer) return;
    var rangeInput1 = priceContainer.querySelectorAll(".range-input input");
    var priceInput1 = priceContainer.querySelectorAll(".price-input input");
    var range1 = priceContainer.querySelector(".slider .progress");
    var errorText2 = document.querySelector(".price-input .error-text._2");
    priceInput1.forEach(function (input) {
      input.addEventListener("input", function (e) {
        var minPrice = parseInt(priceInput1[0].value),
          maxPrice = parseInt(priceInput1[1].value);
        var isInRangeMin = minPrice <= rangeInput1[1].max && minPrice >= rangeInput1[1].min;
        var isInRangeMax = maxPrice <= rangeInput1[1].max && maxPrice >= rangeInput1[1].min;
        if (isInRangeMin === false || isInRangeMax === false) {
          errorText2.classList.add('active');
          errorText2.textContent = "\u0432\u0432\u0435\u0434\u0438\u0442\u0435 \u0447\u0438\u0441\u043B\u043E \u043E\u0442 ".concat(rangeInput1[1].min, " \u0434\u043E ").concat(rangeInput1[1].max);
        } else {
        ...
        input.addEventListener('blur', function (e) {
          if (input.className === "input-min") {
            input.value = rangeInput1[0].min;
          } else {
            input.value = rangeInput1[0].max;
          }
          errorText2.classList.remove('active');
        });
      });
    });
    rangeInput1.forEach(function (input) {
      input.addEventListener("input", function (e) {
        var minVal = parseInt(rangeInput1[0].value),
